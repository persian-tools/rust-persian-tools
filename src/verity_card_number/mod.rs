...
pub fn verify_card_number<S>(digits: S) -> bool where S: Into<String> {
    let digits: String = digits.into();

    if digits.parse::<u64>().is_err(){ return false; }
    if digits.len() < 16 { return false; } // TODO why not digits.len() != 16
    if digits[1..10].parse::<i32>().unwrap() == 0 { return false; }
    if digits[10..16].parse::<i32>().unwrap() == 0 { return false; }
	...
	for (i,digit) in digits.chars().enumerate() {
		...
		sum += if sub_digit  > 9 { sub_digit - 9 } else{ sub_digit };
	}
