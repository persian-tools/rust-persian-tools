use super::farsi_chars::{ FA_TEXT, FA_COMPLEX_TEXT };
...
pub fn has_persian<S>(str: S, is_complex: bool) -> bool where S: Into<String>{
...
mod tests {
    ...
    fn has_persian_test() {
		...
		assert_eq!(has_persian("آیا سیستم میتواند گزینه های دیگری را به اشتباه به عنوان متن فارسی تشخیص دهد؟",false),true);
		assert_eq!(has_persian("This text includes فارسی",false),true);
		assert_eq!(has_persian("Это персидский س текст?",false),true);
		assert_eq!(has_persian("أكد رئيس اللجنة العسكرية الممثلة لحكومة الوفاق أراضي البلاد.",false),true);

		assert_eq!(has_persian("Lorem Ipsum Test",false),false);
		assert_eq!(has_persian("これはペルシア語のテキストですか",false),false);
